...
class Button:
    ...
    def __init__(self, pin, callback=None,callback_long=None, debounce_time=100, long_press_time=500, mode=PUSH): 
        self.button = pin
        ...
        self.val = 1
        if mode==PUSH:
            self.button.irq(trigger=Pin.IRQ_FALLING , handler=self.button_handlerFalling)
        else:    
    ...
    def button_handlerFalling(self,pin):
        self.prev_val=self.val
        self.val=pin.value()
        current_time = time.ticks_ms()
        if self.val == 0 and time.ticks_diff(current_time, self.last_press_time) > self.long_press_time: # Кнопка натиснута
            self.last_press_time = current_time
            self.callback(pin,self)                    
